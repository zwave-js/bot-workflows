...
// embeddings) consumed by the answer pipeline, <records-out> the full
// feedback records consumed by updateDocsFeedbackIssue.cjs.

const fs = require("node:fs/promises");
const path = require("node:path");
const { DOCS_ANSWER_COMMENT_TAG } = require("../answer/gate.cjs");
const {
	DOCS_ANSWER_METADATA_TAG,
	DOCS_ANSWER_METADATA_VERSION,
} = require("../answer/post.cjs");
const { config } = require("../config.cjs");
const { ghGraphql } = require("../lib/githubApi.cjs");
const { EMBEDDING_MODEL, embedBatched } = require("../indexes/localEmbeddings.cjs");
...
async function main() {
	...
	// The suppression list makes the answer pipeline demote responses
