...
// live in the consumer repo's .github/zwave-js-bot.config.json so the same
// shared scripts can run in every consumer; only the JSON differs between
// repos, this loader is byte-identical. Validation runs at require time and
// throws on a missing, unknown, or wrong-typed key, so a bad config fails the
// job loudly instead of silently changing what the bot does.

const fs = require("node:fs");
const path = require("node:path");

// GITHUB_WORKSPACE points at the consumer repo checkout the workflow runs in
const CONFIG_PATH = path.join(
	process.env.GITHUB_WORKSPACE || process.cwd(),
	".github/zwave-js-bot.config.json",
);
...
const OPTIONAL_GROUPS = new Set(["redirects", "cache"]);
